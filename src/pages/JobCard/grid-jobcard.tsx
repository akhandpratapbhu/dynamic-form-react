import { useState, ChangeEvent, MouseEvent, SetStateAction } from "react";
import {
  Box,
  Paper,
  TextField,
...
const JobCardTable: React.FC = () => {
  ...
  return (
    <>
      <div>
        <button onClick={() => navigate("/create-job-card")} style={{ color: "white", backgroundColor: "green", right: "20px" }}>Add Form</button>
      </div>
        ...
        >
          ...
          <Box sx={{ display: "flex", gap: 1 }}>
            <Button
              ...
              onClick={(e: { currentTarget: SetStateAction<HTMLButtonElement | null>; }) => setManageColumnsAnchor(e.currentTarget)}
            >
              Manage Columns
            </Button>
            ...
            <Button
              ...
              onClick={(e: { currentTarget: SetStateAction<HTMLButtonElement | null>; }) => setFilterAnchor(e.currentTarget)}
            >
              Filter
            </Button>
              ...
              {columns.map((column) => (
                <MenuItem key={column.id} dense>
                  <TextField
                    ...
                    onChange={(e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => handleSearchColumnWise(column.id, e)} // Handle change
                    InputProps={{
                      startAdornment: (
                        <InputAdornment position="start">
                          <Search fontSize="small" />
            ...
            <TableBody>
              {sortedData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => {
                ...
                return (
                  <TableRow
                    hover
                    onClick={(event: MouseEvent<HTMLTableRowElement, globalThis.MouseEvent>) => handleClick(event, row.jobId)}
                    role="checkbox"
